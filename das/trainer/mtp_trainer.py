...
from das.utils import AutoRepr, generate_mtp_untrained_model, get_template


@AutoRepr
class MTPTrainer(Worker):
    def __init__(
        self,
        min_dist=1.0,
        max_dist=6.0,
        ...
        **kwargs,
        ...
        self.logger = logging.getLogger("das.trainer.mtp_trainer.MTPTrainer")
    ...
    def gen_unfitted_model(self, fn):
        ...
        write_unfitted_model(self.min_dist, self.max_dist, species_count, self.model_index, fn, self.radial_basis_size)
    ...
    def prepare_run(self):
        ...
        for ii in range(self.n_models):
            ...
            if self.train_from_prev_model:
                ...
                self.gen_unfitted_model(tmp_path / "unfitted.mtp")
            self.dataset.write_mtp_data(tmp_path / "trainset.cfg")
    ...
    def post_run(self):
        ...
        if not self.driver.convergence:
            ...
            self.logger.info(f"converged, copy model from {prev_model_dir} to {model_dir}")
...
def write_unfitted_model(min_dist, max_dist, species_count, model_index, output_fn, radial_basis_size=8):
    mtp_model_str = generate_mtp_untrained_model(min_dist, max_dist, species_count, model_index, radial_basis_size)
    with open(output_fn, "w") as f0:
